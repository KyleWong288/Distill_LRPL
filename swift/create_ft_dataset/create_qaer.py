...
def main(data_path, output_file):
    ...
    for sample in tqdm(input_data):
        clean_code = extract_clean_code(sample["prompt"] + sample["answer"], sample["entry_point"])
        ...
        for i in range(MAX_SAMPLES):
            ...
            # Evaluate response
            clean_code = extract_clean_code(repair, sample["entry_point"])
            run_code = clean_code + "\n" + sample["test"]
            has_error = evaluate_answer(run_code, TEST_DIR)
            if not has_error:
                print("------------------------------------------------PROMPT------------------------------------------------")
                print(prompt)
                print("------------------------------------------------REPAIR------------------------------------------------")
