...
# Pipelined self repair with a base model
def main():
    ...
    for round_num in tqdm(range(1, args.num_rounds+1)):
        ...
        output_file = f"./output/{args.dataset}/{args.model_name}/base/r{round_num}.json"
        ...
        eval_file = f"./eval/{args.dataset}/{args.model_name}/base/r{round_num}.json"
        repair_file = f"./repair/{args.dataset}/{args.model_name}/base/r{round_num}.json"
        evaluation_repair(output_file, eval_file, repair_file, NUM_SAMPLES, args.test_dir)


# Generates the repair with a non fine-tuned model
if __name__ == "__main__":
    ...
    parser.add_argument("--test_dir", type=str, required=True, help="Directory to compile and execute test files in")
