...
# Runs the generated answer, returns error message or None
def evaluate_answer(prompt, answer, test):
    code = extract_clean_code(prompt + "\n" + answer)
    code = code + "\n" + test
    temp_file = "temp.pl"
...
def predict(model, tokenizer, gen_config, sample):
    ...
    for _ in range(MAX_SAMPLES):
        ...
        error = evaluate_answer(prompt, answer, sample["test"])
        if error:
            print("-----------------------------------------------NEW-------------------------------------------------")
            print(prompt)
            print(answer)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_new_tokens", type=int, default=400, help="The maximum number of tokens to generate")
