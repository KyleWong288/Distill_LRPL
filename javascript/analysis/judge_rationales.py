...
def create_judge_prompt(incorrect_code, error, rationale):
    instruction = "You are given an incorrect javascript function, an error message, and a rationale to fix the error. Classify if the rationale is 'Good' or 'Bad'. If the rationale provides enough detail to fix the code, output 'Good'. Otherwise, output 'Bad'.\n"
...
# 1 = good, 0 = bad, 0 invalid response (rare)
...
def main():
    ...
    for DATASET in datasets:
        for MODEL in models:
            ...
            # For each sample, we generate 20 judgements
            output = []
            for idx, question in enumerate(input_data):
                all_judgements = []
                for i in range(NUM_SAMPLES):
                    judgements = []
                    if question["errors"][i] and not has_typo(question["rationales"][i]):
